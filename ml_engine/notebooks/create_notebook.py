    ...
    "cells": [
        ...
        },
        {
            ...
            "source": [
                ...
                "\n",
                "Bộ dữ liệu `ML-EdgeIIoT-dataset.csv` đã sẵn sàng và được tích hợp trực tiếp vào pipeline huấn luyện của hệ thống. Bạn có thể kích hoạt quá trình tối ưu hóa siêu tham số (Optuna HPO 5-Fold Stratified Cross-Validation) kết hợp huấn luyện Final Model qua entrypoint `train.py`:\n",
                "\n",
                "```bash\n",
                "# 1. Tối ưu hóa Random Forest với 20 trials trên dataset Edge-IIoTset:\n",
                "python ml_engine/train.py --classifier random_forest --optuna --n-trials 20 --cv 5 --dataset \"ml_engine/datasets/Edge-IIoTset dataset/Selected dataset for ML and DL/ML-EdgeIIoT-dataset.csv\"\n",
                "\n",
                "# 2. Tối ưu hóa Decision Tree và tự động xuất C Header TinyML cho ESP32:\n",
                "python ml_engine/train.py --classifier decision_tree --optuna --n-trials 25 --cv 5 --dataset \"ml_engine/datasets/Edge-IIoTset dataset/Selected dataset for ML and DL/ML-EdgeIIoT-dataset.csv\"\n",
                "\n",
                "# 3. Huấn luyện mô hình kết hợp Ensemble Voting (RandomForest + ExtraTrees + GradientBoosting):\n",
                "python ml_engine/train.py --classifier ensemble_voting --optuna --n-trials 15 --cv 5 --dataset \"ml_engine/datasets/Edge-IIoTset dataset/Selected dataset for ML and DL/ML-EdgeIIoT-dataset.csv\"\n",
                "\n",
